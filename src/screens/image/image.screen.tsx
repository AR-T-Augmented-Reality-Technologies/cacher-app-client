...
export const ImageScreen = ({ navigation }: ImageScreenProps) => {
    ...
    <div>
        ...
        {/* like button */}
        <button className={`w-16 h-16 rounded-full text-xs text-black bg-white font-bold border-solid border-2 ${isLiked ? 'border-custom-blue' : 'border-black'} text-center fixed bottom-20 right-2`}onClick={likePost}>
        <svg fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6 mx-auto my-auto"><path stroke-linecap="round" stroke-linejoin="round" d="M6.633 10.5c.806 0 1.533-.446 2.031-1.08a9.041 9.041 0 012.861-2.4c.723-.384 1.35-.956 1.653-1.715a4.498 4.498 0 00.322-1.672V3a.75.75 0 01.75-.75A2.25 2.25 0 0116.5 4.5c0 1.152-.26 2.243-.723 3.218-.266.558.107 1.282.725 1.282h3.126c1.026 0 1.945.694 2.054 1.715.045.422.068.85.068 1.285a11.95 11.95 0 01-2.649 7.521c-.388.482-.987.729-1.605.729H13.48c-.483 0-.964-.078-1.423-.23l-3.114-1.04a4.501 4.501 0 00-1.423-.23H5.904M14.25 9h2.25M5.904 18.75c.083.205.173.405.27.602.197.4-.078.898-.523.898h-.908c-.889 0-1.713-.518-1.972-1.368a12 12 0 01-.521-3.507c0-1.553.295-3.036.831-4.398C3.387 10.203 4.167 9.75 5 9.75h1.053c.472 0 .745.556.5.96a8.958 8.958 0 00-1.302 4.665c0 1.194.232 2.333.654 3.375z" />
        </svg>Like</button>

        {/* comment button */}
        <button className="w-16 h-16 rounded-full text-tiny text-black bg-white font-bold border-solid border-2 border-black text-center fixed bottom-2 right-2" onClick={displayComments}><svg fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6 mx-auto my-auto">
        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
        </svg>Comment</button>

        {/* options button */}
        <button className={`w-16 h-16 rounded-full text-xs text-black bg-white font-bold border-solid border-2 ${showOptions ? 'border-custom-blue' : 'border-black'} text-center fixed bottom-2 left-2`}onClick={displayOptions}>
    ...
    {showOptions && (
        <>
            {/* Profile button */}
            <button className="w-16 h-16 rounded-full text-xs text-black bg-white font-bold border-solid border-2 border-black text-center fixed bottom-40 left-2 transition duration-500 ease-in-out" onClick={() => {navigation.navigate('Profile')}}><svg fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" className="w-6 h-6 mx-auto my-auto">
            <path stroke-linecap="round" stroke-linejoin="round" d="M17.982 18.725A7.488 7.488 0 0012 15.75a7.488 7.488 0 00-5.982 2.975m11.963 0a9 9 0 10-11.963 0m11.963 0A8.966 8.966 0 0112 21a8.966 8.966 0 01-5.982-2.275M15 9.75a3 3 0 11-6 0 3 3 0 016 0z" />
            </svg>Profile</button>

            {/* Settings button */}
            <button className="w-16 h-16 rounded-full text-xs text-black bg-white font-bold border-solid border-2 border-black text-center fixed bottom-20 left-2 transition duration-500 ease-in-out" onClick={() => {navigation.navigate('')}}><svg fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" className="w-6 h-6 mx-auto my-auto">
            ...
            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>Settings</button>
        </>
      )}
    ...
    {showLikeCount && (
        <>
            {/* Like count popup */}
            <div className={`${showLikeCount ? 'opacity-100' : 'opacity-0'} transition-opacity ease-in-out duration-300`}>
            <div className="bg-white p-3 rounded-lg shadow-lg fixed bottom-24 right-20 border border-black">
            <svg fill="none" viewBox="0 0 24 24" stroke-width="1.5" className="w-6 h-6 mx-auto my-auto stroke-like-button-orange">
    ...
    {showComments && (
            <>
            {/* Comments popup */}
            <div ref={popupRef} className={`${showComments ? 'opacity-100' : 'opacity-0'} transition-opacity ease-in-out duration-300`}>
            <div className="h-screen bg-white border-solid border-2 border-black fixed top-48 left-2 right-2 rounded-t-3xl border-b-0">

            {/* Post description*/}
            <div className="grid grid-rows-2">
            <div className="row-start-1">
                <div className="pt-5 px-5 text-s break-normal">These pets are so cute!</div>
            </div>
            <div className="row-start-2">
            ...
            {comments.map((comment, index) => (
                <div key={index} className="bg-white border-solid border border-black break-normal h-auto rounded-lg mb-5 grid grid-cols-5 grid-rows-2">
                <p className="break-normal px-2 py-2 col-start-1 col-span-3 row-start-1 text-sm overflow-hidden text-justify" style={{ wordWrap: 'break-word' }}>{comment} </p>
                <p className="break-normal px-2 py-2 col-start-1 col-span-3 row-start-2 text-sm overflow-hidden text-justify" style={{ wordWrap: 'break-word' }}>({formatTime(commentTimes[index])})</p>
                <img src="images/avatar-image.jpg" className="border-solid border-2 border-black rounded-lg w-3/4 float-right col-start-5 cols-span-1 mt-2"></img>
                <p className="col-start-4 col-span-2 pt-5 pr-3 text-right text-sm">User123456</p>
                </div>
            ))}
            </div>
